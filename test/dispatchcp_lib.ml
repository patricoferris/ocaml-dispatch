...
(* 
  This implementation performs reads and writes 
  asynchronously but uses a serial queue to ensure that
  the reads and writes are serialised.
*)
...
let copy_file block_size _queue_depth in_path out_path () =
  ...
  Dispatch.Io.with_read serial read_io group
    ~f:(fun data ->
      (* Printf.printf "Data size: %i bytes\n" (Dispatch.Data.size data); *)
